...
export function HostMissionEngine({ guests, eventType }: HostMissionEngineProps) {
  ...
  return (
    <section className="pn-mission-card md:col-span-4">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <p className="text-sm font-bold uppercase text-party-teal">Mission Engine v2</p>
          <h2 className="mt-1 text-2xl font-bold text-party-soft">
            Live Participation Rounds
          </h2>
          <p className="mt-2 text-sm font-semibold text-party-gold">
            {eventType} template
          </p>
        </div>
        <div className="grid gap-2 sm:grid-cols-[auto_auto] sm:items-end">
          <div>
            <p className="mb-2 text-sm font-semibold text-slate-200">Round timer</p>
                ...
                <button
                  ...
                  className={`min-h-11 rounded-md border px-3 text-sm font-bold transition ${
                    durationMinutes === minutes
                      ? "border-party-teal bg-party-teal text-party-navy"
                      : "border-party-blue/40 text-party-soft hover:border-party-teal"
                  }`}
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(18rem,1.1fr)]">
        <form onSubmit={handleSaveMission} className="space-y-4">
          <div className="rounded-md border border-party-blue/25 bg-party-navy/70 p-3">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
              <div>
                <p className="text-sm font-bold uppercase text-party-teal">
                  Default Template
                </p>
                <p className="mt-1 text-sm font-semibold text-party-soft">
          ...
          <label className="block">
            <span className="text-sm font-semibold text-party-soft">Mission</span>
            <textarea
              required
              value={prompt}
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
            <label className="block">
              <span className="text-sm font-semibold text-party-soft">Category</span>
              <select
                value={category}
                onChange={(event) =>
            ...
            <label className="flex min-h-12 items-center gap-3 rounded-md border border-party-blue/35 px-4 text-sm font-bold text-party-soft">
              <input
                type="checkbox"
                checked={saveAsTemplate}
                onChange={(event) => setSaveAsTemplate(event.target.checked)}
                className="h-5 w-5 accent-party-teal"
              />
              Save template
            </label>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {missionExamples.map((example) => (
              <button
                key={example.prompt}
                type="button"
                className="rounded-md border border-party-blue/35 bg-party-blue/10 px-3 py-2 text-left text-sm font-semibold text-slate-100 transition hover:border-party-teal"
        ...
        <div>
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-lg font-bold text-party-soft">Mission Library</h3>
            <p className="text-sm font-semibold text-party-teal">
              {activeMissions.length} active
            </p>
          </div>

          {templates.length ? (
            <div className="mt-3 rounded-md border border-party-blue/25 bg-party-navy/70 p-3">
              <p className="text-sm font-bold uppercase text-party-teal">
                Reusable templates
              </p>
              <div className="mt-2 flex gap-2 overflow-auto pb-1">
                {templates.map((mission) => (
                  <button
                    key={mission.id}
                    type="button"
                    className="min-w-48 rounded-md border border-party-blue/35 bg-party-blue/10 p-3 text-left text-sm font-semibold text-slate-100 transition hover:border-party-teal"
                    onClick={() => applyTemplate(mission)}
                  >
                    <span className="block text-party-gold">{mission.category}</span>
                    {mission.prompt}
                  </button>
                ))}
              </div>
          ...
          <div className="mt-3 max-h-[30rem] space-y-2 overflow-auto pr-1">
            {missions.length ? (
              missions.map((mission) => (
                <article
                  key={mission.id}
                  className="rounded-md border border-party-blue/25 bg-party-navy/70 p-3"
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="grid shrink-0 gap-2">
                      <button
                        type="button"
                        className="rounded-md border border-party-blue/35 px-3 py-2 text-sm font-bold text-party-soft transition hover:border-party-teal"
              ...
              <p className="pn-empty-state p-5">
          ...
          <Link
            ...
            className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-md border border-party-blue/40 px-4 py-2 text-center text-sm font-bold text-party-soft transition hover:border-party-teal"
          >
...
function StatTile({
  ...
  return (
    <div className="pn-stat-card p-4">
      ...
      {detail ? <p className="mt-1 text-sm font-semibold text-slate-300">{detail}</p> : null}
    </div>
  );
}
...
function TemplateList({
  ...
  return (
    <div className="rounded-md border border-party-blue/25 bg-party-navy/55 p-3">
      <p className="text-xs font-bold uppercase text-party-gold">{title}</p>
      <div className="mt-2 space-y-2">
        {items.map((item) => (
          <button
            key={item.prompt}
            type="button"
            className="w-full rounded-md border border-party-blue/25 bg-party-blue/10 px-3 py-2 text-left text-xs font-semibold leading-5 text-slate-100 transition hover:border-party-teal"
